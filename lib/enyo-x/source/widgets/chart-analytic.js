  ...
  */
    ...
    */
    ...
    create: function () {
      ...
      this.getValue().fetch({
        ...
        //data : {mdx : "select NON EMPTY {  [Measures].[Amount, Profit Gross]} ON COLUMNS, NON EMPTY {[Product.Product Code].Children} ON ROWS from [SODelivery] where [Customer.Customer Code].[TTOYS]"},
  ...
  enyo.kind(
      ...
      if (collection.models.length > 0) {
        ...
        for (var propt in collection.models[0].attributes) {
          ...
          if (propt.indexOf("[Measures]") !== -1) {
            ...
            /**
            collection.models.map(function (theModel) {
              values.push({ x: theModel.attributes[2], y: theModel.attributes[propt]});
    ...
    plot: function () {
      ...
      var chart = chartFunc().height(180);
